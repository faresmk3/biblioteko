...
def main(global_config, **settings):
    ...
    with Configurator(settings=settings) as config:
        ...
        # Routes de gestion des œuvres
        config.add_route('valider_form', '/valider_form')   # Validation avec options
        config.add_route('valider', '/valider/{id}')        # <--- C'EST CELLE QUI MANQUAIT (Validation simple)
        config.add_route('rejeter', '/rejeter/{id}')        # Rejet
        config.add_route('download_md', '/download_md/{id}') # Markdown IA
