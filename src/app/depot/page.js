...
export default function DepotPage() {
  ...
  // FONCTION : Télécharger le Markdown
  const handleDownload = () => {
    if (!markdownContent) {
      setMessage({ type: 'warning', text: '⚠️ Aucun contenu à télécharger' });
      return;
    }
    ...
    link.href = url;
    link.download = `${formData.titre.replace(/[^a-z0-9]/gi, '_') || 'document'}.md`;
    ...
    setMessage({ 
      type: 'success', 
      text: '✅ Markdown téléchargé ! Vous pouvez maintenant le soumettre ou annuler.' 
    });
  };

  // FONCTION : Annuler tout le processus
  const handleCancel = () => {
    if (window.confirm('⚠️ Êtes-vous sûr de vouloir annuler ? Toutes les modifications seront perdues.')) {
      ...
      setMessage({ 
        type: 'info', 
        text: '🔄 Processus annulé. Vous pouvez recommencer.' 
      });
    }
  };
          ...
          <Card.Body>
            ...
            <div className="mt-3">
              ...
              <Form.Text className="text-muted d-block mt-2">
                Sauvegardez une copie locale avant de soumettre
              </Form.Text>
            </div>
          </Card.Body>
        </Card>
